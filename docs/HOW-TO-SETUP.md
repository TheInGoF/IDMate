...
| Topic prefix  | `tele/vw_nox`           | determines the **device name** (see section 3)         |
| AES-256 key   | `<the same 64 hex>`     | = `MQTT_AES_KEY` from `.env`                            |

The stick then publishes to **`<topic-prefix>/data`**, e.g. `tele/vw_nox/data`
([IDTelemetry/src/mod_mqtt.cpp](../../IDTelemetry/src/mod_mqtt.cpp)). The MQTT client ID is
derived automatically from the last topic segment (`tele/vw_nox` → `vw_nox`).
...
2. Click "Add vehicle" and fill in:
   ...
   | **Device**            | `vw_nox`        | **must exactly match the topic-prefix suffix**        |
   | VIN                   | `WVW...`        | optional                                              |
   | Battery capacity (kWh)| `77`            | optional, for range/efficiency calculation           |

   The **device field is the key**: if the stick uses `tele/vw_nox`, this must read `vw_nox`.

3. Save. From now on, incoming `tele/vw_nox/data` telegrams are mapped to this vehicle.
...
- **Different topic prefix** per stick, e.g. `tele/vw_nox` and `tele/vw_two`.
- One `vehicles` row per stick with the matching `device` (`vw_nox`, `vw_two`).
