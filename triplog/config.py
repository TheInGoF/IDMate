...
# ── InfluxDB field types (measurement "v") — ONE truth for all writers ──
# Field types are FIXED once a field has been written: a deviating type
# returns 422 field-type-conflict and the ENTIRE batch/day is discarded
# (real incidents: commits 9ebbbad, 73afef1, d951af9 = data loss).
# This table must be used identically by the MQTT bridge, replay (app.py)
# and the backfill import (import_job.py) — nobody casts Influx fields
# by hand anymore. Derived from the binary decoder (_BIN_FIELDS in app.py):
# fields WITHOUT a divisor are integers (with divisor: la, lo, s, p, r, ca, kw,
# od = FLOAT).
#
# CAUTION 'v' (speed): the correct type is INSTALLATION-DEPENDENT. Some
# buckets have established 'v' as float (incident 73afef1 = exactly this
# installation), others as integer (incident d951af9). A hardcoded value
# therefore ping-pongs endlessly between both worlds and breaks the other each time.
# Solution: overridable via ENV. Default = float for 'v' (bucket of this
# installation, confirmed by 73afef1 + another 422 on 2025-12-31). An
# installation whose bucket holds 'v' as integer sets:
#   INFLUX_INT_FIELDS=hd,u,i,c,dc,bt,et,pk,ls,bd,lp,v
# Format: comma-separated field names. Empty string = ALL fields float.
...
# extended automatically by import jobs and the "fill gaps" button.
