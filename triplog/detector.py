...
def save_trips(db, trips):
    ...
    for t in trips:
        ...
        # Funkloch-merge: data dropouts (tunnel / dead-zone) make the detector
        ...
        if split:
            ...
            log.info("Trip %d Funkloch-merged: gap-end %s, resumed %s",
...
def cleanup_db(db):
    ...
    # they surface in the readings editor's "Nicht zugeordnet" view so they can
...
def _run_scan(db, client, scope_from, scope_to):
    ...
    # to ~36h and bounded by the manual "abgerechnet bis" freeze in app side.
    try:
