...
# ProxyFix: respektiert X-Forwarded-For/-Proto/-Host vom Reverse-Proxy
# (Nginx/Caddy/Traefik vor dem Container). Ohne diesen Wrapper sieht Flask
# als Client-IP nur die Proxy-IP — Login-Rate-Limits, CSRF-Origin-Checks und
# Audit-Logs werden damit wertlos. x_for=1 vertraut einer Proxy-Hop-Tiefe;
# bei mehrstufigen Proxies entsprechend hochziehen.
...
def _invoice_safe(value):
    """Rechnungs-Vorlagenfelder sicher rendern: alles HTML-escapen, nur <br>
    als Zeilenumbruch zulassen. Ersetzt das frühere |safe, über das ein User
    <script>/<img onerror> als persistentes Stored-XSS hinterlegen konnte.

    Zusätzlich werden contenteditable-Artefakte normalisiert: Browser speichern
    Enter als <div>…</div>/<p>…</p>-Bloecke und Leerzeichen als &nbsp; — Bestands-
    werte aus der innerHTML-Speicher-Aera erschienen sonst als woertliches
    "<div>" im Feld. Block-Anfaenge werden zu <br>, Block-Enden entfernt."""
    ...
    esc = esc.replace("&amp;nbsp;", " ")
    # <div><br></div>-Leerzeilen ergeben 3 Umbrueche -> auf max. 2 eindampfen
    import re as _re
    esc = _re.sub(r"(?:<br>){3,}", "<br><br>", esc)
    # fuehrende Umbrueche (erstes <div> direkt am Anfang) nicht anzeigen
...
# Remember-Me-Cookie genauso härten wie das Session-Cookie — Flask-Login-
# Defaults sind Secure=False/SameSite=None, sonst kann das 7-Tage-Auth-Token
# über einen HTTP-Request abfließen.
...
@app.before_request
def _gen_csp_nonce():
    """Per-Request-Nonce fuer Inline-<script> (FIXES 7.2). Mit Nonce in
    script-src ignorieren CSP3-Browser 'unsafe-inline' — Inline-Handler-
    Attribute sind damit blockiert; 'unsafe-inline' bleibt nur als Fallback
    fuer Alt-Browser im Header."""
...
UI_THEMES = {"", "light", "gt"}  # '' = dunkel (Standard)
...
# Zweiter Zähler pro Username (lowercase/strip) — drosselt Brute-Force
# über rotierende IPs hinweg. Gleicher Mechanismus, Username als Key.
_login_attempts_user = {}  # {username_key: [timestamp, ...]}
_LOGIN_MAX = 5
_LOGIN_COOLDOWN = 60  # seconds

def _user_key(username):
    """Normalisierter Rate-Limit-Key für einen Username (lowercase/strip)."""
    return (username or "").strip().lower()

def _is_rate_limited(ip):
    now = time.time()
    # Remove old entries; leere Keys ganz entfernen, sonst wächst das Dict bei
    # rotierenden IPs unbegrenzt (Memory-Leak).
...
def _safe_next(target):
    """Open-Redirect-Schutz: nur lokale Pfade als Redirect-Ziel zulassen.
    Lehnt absolute/protokoll-relative URLs und den Backslash-Trick ab."""
...
def require_login():
    ...
    if request.endpoint in ("auth.login", "static", "charges.charge_webhook", "auth.setup", "auth.setup_2fa", "auth.login_totp", "healthz"):
        return
    if _setup_required:
...
def get_purpose_meta(db, user_id=None, device=None):
    ...
    FIXES 15.2 — fahrzeug-getriebene Sichtbarkeit: ist *device* gesetzt, werden
    nur Zwecke geliefert, die für dieses Fahrzeug relevant sind: bereits auf einem
    Trip des Fahrzeugs verwendet (automatisch), ODER `is_main` (angepinnter
    Default), ODER dem User explizit zugewiesen (purpose_visibility, Admin-Matrix).
    So zeigt jedes Fahrzeug nur sein eigenes Vokabular. *device=None* (Default)
    liefert den vollen Katalog — für die Admin-/Verwaltungssicht (sieht alles).
    """
    if device is None:
        rows = db.execute(
...
def get_preset_values(db, field, device=None, user_id=None):
    """Vorschlagswerte (destination | visit_reason). FIXES 15.2: fahrzeug-getrieben
    analog zu get_purpose_meta — auf dem Fahrzeug verwendete Werte (aus trips) plus
    explizit zugewiesene (preset_value_visibility). device=None = voller Katalog."""
    if device is None:
        rows = db.execute(
...
def allowed_vehicle_ids(db, user):
    """FIXES — Pro-User-Fahrzeug-Sichtbarkeit: Menge der Fahrzeug-IDs, die *user*
    sehen darf. None = keine Einschraenkung (alle). Admins sehen immer alle;
    User ohne Zuweisungszeilen ebenfalls alle (Rueckwaertskompat)."""
...
def active_vehicle(db=None):
    ...
    Beachtet die Pro-User-Fahrzeug-Sichtbarkeit (allowed_vehicle_ids): ein nicht
    erlaubtes Aktiv-/Default-Fahrzeug wird ignoriert, Fallback = erstes erlaubtes."""
    close = False
    if db is None:
        db = get_db()
        close = True
...
def set_active_vehicle():
    ...
    vehicle_id = data.get("vehicle_id")
    db = get_db()
    # Kein Aktivsetzen eines nicht erlaubten Fahrzeugs (Pro-User-Sichtbarkeit).
    allowed = allowed_vehicle_ids(db, current_user)
    if vehicle_id and allowed is not None and int(vehicle_id) not in allowed:
        db.close()
        return jsonify({"error": "Kein Zugriff auf dieses Fahrzeug"}), 403
...
def analysis_page():
    ...
    # Default to the last 30 days when no range is set yet — otherwise the page
    # would request today..today and a first-time visitor sees an empty analysis
...
def trip_coords():
    ...
    # Honour the date toolbar when a range is set; otherwise fall back to the
    # last 180 days (all-time would be too many points for the heatmap).
    if date_from and date_to:
        range_clause = f'|> range(start: {date_from}T00:00:00Z, stop: {date_to}T23:59:59Z)'
    else:
        range_clause = '|> range(start: -180d)'
    client = detector.get_influx()
    if not client:
        return jsonify([])
    coords = []
...
def _reassign_trips_to_location(db, loc_id):
    loc = db.execute(
        "SELECT name, lat, lon, radius_m FROM locations WHERE id = ?",
        (loc_id,),
    ).fetchone()
    if not loc:
        return {"trips_start": 0, "trips_end": 0}
...
def delete_location(loc_id):
    db = get_db()
    loc = db.execute(
        "SELECT name, lat, lon, radius_m FROM locations WHERE id = ?", (loc_id,)
    ).fetchone()
    ...
    if loc:
        ...
        # bring back the street addresses.
...
def _parse_local(s):
    """ISO-Zeitstring → tz-bewusstes datetime in Europe/Berlin.
    Gespeicherte Trip-Zeiten sind Lokalzeit ohne Offset."""
...
def _trip_telemetry_points(trip, max_points=300):
    """Zeitlich sortierte Telemetriepunkte einer Fahrt für die Split-UI:
    [{t: lokal-ISO, lat, lon, odo, soc}, ...]. InfluxDB für normale Fahrten,
    gpx_waypoints für GPX-Importe. Vorwärts-gefüllt, downgesampelt."""
    pts = []
    if trip.get("is_gpx"):
        db = get_db()
        wps = db.execute(
...
def mqtt_replay():
    ...
    for m in _mqtt_messages:
        ...
        # Feldtypen über die zentrale Tabelle erzwingen (siehe
        # config.INFLUX_INT_FIELDS) — sonst 422 field-type-conflict und der
        # ganze Batch wird verworfen.
...
def debug_page():
    ...
    if device not in valid_devices:
        db.close()
        return "<h1>Unbekanntes device</h1>", 400
...
def _influx_soc_for_session(device, start_local, end_local):
...
def _charge_scope_floor(db, since):
    ...
    floor = str(since)
    row = db.execute(
        "SELECT MIN(cs.start_time) AS m FROM charge_sessions cs "
        "JOIN charge_readings cr ON cr.session_id = cs.id "
        "WHERE cs.is_external = 0 AND cr.timestamp >= ?", (since,)
    ).fetchone()
    if row and row["m"] and row["m"] < floor:
        floor = row["m"]
    prev = db.execute(
        "SELECT start_time FROM charge_sessions WHERE is_external = 0 "
        "AND datetime(start_time) <= datetime(?) "
        "ORDER BY datetime(start_time) DESC LIMIT 1", (since,)
    ).fetchone()
    if prev and prev["start_time"] and prev["start_time"] < floor:
        floor = prev["start_time"]
    return floor
...
def _rebuild_charge_sessions_impl(db, since=None):
    ...
    # only the car identity was unresolved (e.g. VW-ID-Integration hängt am
    # 8-Uhr-Handshake, fängt erst um 11 Uhr das Kennzeichen). Otherwise the
        ...
        for r in preads:
            ...
            if cur_group and prev is not None:
                try:
                    ...
                    gap = 0
                if gap > _GAP_CHECK_MIN:
                    if device:
                        # Split only if the car actually drove in between.
                        moved = db.execute(
                            "SELECT 1 FROM trips WHERE device = ? "
                            "AND datetime(start_time) >= datetime(?) "
                            "AND datetime(start_time) <  datetime(?) "
    ...
    for plate, reads in groups:
        ...
        # Trip-fallback for missing/stale HA values (VW-ID-Integration hängt):
...
@admin_required  # FIXES 15.2: Tarife sind System-Konfig (Kostenrechnung) — nur Admin
def create_tariff():
    data = request.get_json()
...
@admin_required  # FIXES 15.2: Tarife sind System-Konfig — nur Admin
...
# User-Avatar-Routen: seit dem Profil-Umbau in blueprints/profile.py
# (gleiche URLs). _USER_AVATAR_DIR bleibt hier (Media-Serving + Import).
...
def upload_charge_location_icon(loc_id):
    ...
    if ext not in _ALLOWED_IMAGE_EXTS:
        db.close()
        return jsonify({"error": "Ungültiges Format (erlaubt: PNG, JPG, WEBP, GIF)"}), 400
