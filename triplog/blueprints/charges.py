...
from app import (ENCRYPTED_SETTINGS, _billed_floor, _decrypt_setting,
                 ...
                 _settings_fernet, active_device, active_vehicle, admin_required,
                 csrf, get_bat_kwh, get_db, get_language, haversine_m, log,
                 match_charge_location, match_location, rebuild_charge_sessions)
...
def charge_webhook():
    ...
    if not token:
        log.warning("charge_webhook: 503 — CHARGE_WEBHOOK_TOKEN ist leer (env nicht gesetzt?)")
    ...
    if not hmac.compare_digest(auth, expected):
        log.warning("charge_webhook: 401 — Authorization-Header passt nicht (len got=%d, expected=%d, starts_with_Bearer=%s)",
                    len(auth), len(expected), auth.startswith("Bearer "))
        return jsonify({"error": "Unauthorized"}), 401

    data = request.get_json(force=True, silent=True)
    if not data:
        log.warning("charge_webhook: 400 — Body ist kein JSON oder leer")
        return jsonify({"error": "JSON erwartet"}), 400

    vehicle = str(data.get("vehicle", "")).strip()
    if not vehicle or vehicle == "free":
        log.info("charge_webhook: skipped — vehicle=%r (HA-input_select scheint auf 'free' oder leer zu stehen)",
                 vehicle)
    ...
    if kwh <= 0:
        log.info("charge_webhook: skipped — kwh=%s (Wallbox liefert 0/negativ; vehicle=%s)",
                 kwh, vehicle)
...
def charges_list():
    ...
    if "from" in request.args or "to" in request.args:
        session["date_from"] = request.args.get("from", "")
        session["date_to"] = request.args.get("to", "")
...
def charges_print():
    ...
    for s in sessions:
        ...
        try:
            ...
            # Ladung endet an einem spaeteren Kalendertag -> Endzeit markieren
...
def charge_billed_until():
    ...
    if val:
        try:
            ...
            return jsonify({"error": "Datum im Format YYYY-MM-TT erwartet"}), 400
    db = get_db()
...
@admin_required  # FIXES 15.2: Sessions global neu aufbauen ist System-Wartung — nur Admin
def charge_rebuild():
    db = get_db()
    rebuild_charge_sessions(db)
