...
from app import (ENCRYPTED_SETTINGS, _decrypt_setting, _encrypt_setting,
                 _find_odo_gaps, _point_in_polygon, _settings_fernet,
                 _translations, _trend_cache, _validate_password, active_device,
                 active_vehicle, admin_required, debug_required,
                 generate_password_hash, get_bat_kwh, get_db, get_language,
                 get_purpose_meta, haversine_m, import_job, log,
                 rebuild_charge_sessions)
...
def admin_values():
    ...
    for loc in locations:
        ...
        for t in trips_with_coords:
            if poly:
                if _point_in_polygon(t["end_lat"], t["end_lon"], poly):
                    count += 1
            elif haversine_m(t["end_lat"], t["end_lon"], loc["lat"], loc["lon"]) <= loc["radius_m"]:
                count += 1
...
def save_settings():
    ...
    @admin_required: settings are global configuration (language, map style,
    tokens, invoice templates) — previously any logged-in user could write
    arbitrary keys (mass assignment) including the invoice_* HTML fields."""
...
@admin_bp.route("/api/admin/ha-export")
@admin_required
def ha_export_get():
    """Home Assistant export config: token, selected devices, vehicle list, URL."""
    db = get_db()
    vehicles = db.execute(
        "SELECT plate, name, device FROM vehicles "
    ...
    url_row = db.execute(
        "SELECT value FROM settings WHERE key = 'ha_export_url'"
    ).fetchone()
    db.close()

    token = ""
    if tok_row and tok_row["value"]:
        token = _decrypt_setting(_settings_fernet(), tok_row["value"])
    try:
        selected = set(json.loads(dev_row["value"])) if dev_row and dev_row["value"] else set()
    except Exception:
        selected = set()
    # Prefer the admin-saved base URL (e.g. the LAN IP). Fall back to however
    # this page was reached — which is the reverse-proxy domain when accessed
    # externally, and only the LAN IP when reached directly. Inside Docker the
    # host's LAN IP cannot be auto-detected, so the field is editable.
    url = url_row["value"] if url_row and url_row["value"] else f"{request.scheme}://{request.host}"
        ...
        "vehicles": [
            {
                "device": v["device"],
                "plate": v["plate"],
                "name": v["name"] or v["plate"],
                "exposed": v["device"] in selected,
            }
            for v in vehicles
        ],
    })
...
@admin_bp.route("/api/admin/ha-export", methods=["POST"])
@admin_required
def ha_export_save():
    """Save selected devices and optionally (re)generate or clear the token.

    Body: {"devices": [...], "action": "save"|"regenerate"|"clear"}.
    """
    ...
    db = get_db()
    db.execute(
        "INSERT INTO settings (key, value) VALUES ('ha_export_devices', ?) "
        "ON CONFLICT(key) DO UPDATE SET value = ?",
        (json.dumps(devices), json.dumps(devices)),
    )

    # Base URL the HA integration should call (e.g. the LAN IP). Empty -> clear,
    # so it falls back to the request-derived URL.
    if "url" in data:
        url = str(data.get("url") or "").strip().rstrip("/")
        if url:
            db.execute(
                "INSERT INTO settings (key, value) VALUES ('ha_export_url', ?) "
                "ON CONFLICT(key) DO UPDATE SET value = ?",
                (url, url),
            )
        else:
            db.execute("DELETE FROM settings WHERE key = 'ha_export_url'")

    token = None
    if action == "regenerate":
        token = secrets.token_urlsafe(24)
        enc = _encrypt_setting(_settings_fernet(), token)
        db.execute(
            "INSERT INTO settings (key, value) VALUES ('ha_export_token', ?) "
            "ON CONFLICT(key) DO UPDATE SET value = ?",
            (enc, enc),
        )
    elif action == "clear":
        db.execute("DELETE FROM settings WHERE key = 'ha_export_token'")
        token = ""

    db.commit()
    db.close()

    result = {"ok": True, "devices": devices}
...
def update_purpose_meta():
    ...
    # FIXES 15.2: if the purpose is new, record the creator + grant them
    # visibility so it shows up in the dropdown immediately (even before it
    # appears on any trip).
...
def add_preset_value():
    ...
    FIXES 15.2: @login_required (was @admin_required) — every user may create
    their own suggestions; created_by + visibility row makes them visible at once."""
    data = request.get_json()
    ...
    if field not in ("destination", "visit_reason") or not value:
        lang = get_language()
        _t = _translations.get(lang, _translations["DE"])
        return jsonify({"error": _t["admin_err_field_value_required"]}), 400
    ...
    # fetch the id of the (possibly already existing) entry, then visibility for the user
...
def _may_manage_value(db, field, name):
    """FIXES 15.6: who may rename/merge/delete a tag value? Admins always;
    regular users only their own entries (created_by). Legacy (created_by NULL)
    and foreign ones -> admin only. Values that only appear in trips (no catalog
    entry) can likewise only be touched by the admin (created_by unknown)."""
...
def admin_rename():
    """Rename a value in all trips. FIXES 15.6: @login_required — everyone may
    rename their own tags, admins all of them."""
    data = request.get_json()
    ...
    if field not in ("purpose", "destination", "visit_reason"):
        return jsonify({"error": _t["admin_err_invalid_field"]}), 400
    if not old_name or not new_name:
        return jsonify({"error": _t["admin_err_name_not_empty"]}), 400

    db = get_db()
    if not _may_manage_value(db, field, old_name):
        db.close()
        return jsonify({"error": _t["admin_err_only_own_tags"]}), 403
    # purpose_visibility has an FK on purpose_meta.name without ON UPDATE
    # CASCADE — defer FK checks for this transaction so parent + child can be
    # renamed consistently (otherwise an FK error on rename).
...
def admin_merge():
    """Merge two values (source → target). FIXES 15.6: @login_required — the
    SOURCE value gets deleted, so check ownership of it (admins all)."""
    data = request.get_json()
    field = data.get("field")
    ...
    if not source or not target:
        return jsonify({"error": _t["admin_err_both_names_required"]}), 400
...
def admin_delete_value():
    ...
    everyone may delete their own tags, admins all of them."""
    data = request.get_json()
    field = data.get("field")
    name = data.get("name", "").strip()
    ...
    if not name:
        return jsonify({"error": _t["admin_js_name_required_short"]}), 400
    if confirm != name:
        return jsonify({"error": _t["admin_err_confirm_mismatch"]}), 400
...
def admin_teslamate_day_compare():
    ...
    # Plausibility checks: no unsafe imports without a sanity pass
    ...
    # Range-check bounds: deliberately set wide so journeys across all of
    # Europe don't show up as outliers — Crete to North Cape, Iceland to the
    # Caucasus. The goal is only "is it still geographically sensible" and "is
    # it a numeric type".
    tm_ranges = {
        "latitude":      (30.0, 72.0),
        "longitude":     (-30.0, 45.0),
...
def admin_teslamate_positions_preview():
    ...
    # Merge: one row per day with TM count, IDMate count, estimated-new
    out = []
    tm_total = 0
    idm_total = 0
    ...
    for r in tm_rows:
        ...
        # Density cap: max 4320 samples/day (1 every 20s over 24h)
...
def delete_charge_range():
    ...
    if id_from is None or id_to is None:
        lang = get_language()
        _t = _translations.get(lang, _translations["DE"])
        return jsonify({"error": _t["admin_err_from_to_required"]}), 400
    db = get_db()
...
def admin_scan_debug():
    ...
    if device not in valid_devices:
        ...
        lang = get_language()
        _t = _translations.get(lang, _translations["DE"])
        return jsonify({"error": _t["admin_err_unknown_device"]}), 400
    ...
    if not client:
        ...
        lang = get_language()
        _t = _translations.get(lang, _translations["DE"])
        return jsonify({"error": _t["err_influx_unreachable"]}), 503
...
def admin_influx_delete():
    ...
    if not device or not timestamps:
        return jsonify({"ok": False, "error": _t["admin_err_device_timestamps_required"]}), 400
    ...
    db.close()
    if device not in valid_devices:
        return jsonify({"ok": False, "error": _t["admin_err_unknown_device"]}), 400

    client = detector.get_influx()
    if not client:
        return jsonify({"ok": False, "error": _t["err_influx_unreachable"]}), 503
...
def list_users():
    ...
    # Per-user vehicle visibility: empty list = all vehicles (no restriction).
    access = {}
...
def set_user_vehicles(user_id):
    """Which vehicles the user may see. Empty list = all (no restriction),
    non-empty list = exactly these (rows replaced)."""
    ...
    try:
        ...
        lang = get_language()
        _t = _translations.get(lang, _translations["DE"])
        return jsonify({"error": _t["admin_err_invalid_vehicle_ids"]}), 400
    db = get_db()
...
def create_user():
    ...
    if not username or not password:
        return jsonify({"error": _t["admin_err_username_password_required"]}), 400
    ...
    if existing:
        db.close()
        return jsonify({"error": _t["admin_err_username_exists"]}), 400
...
def delete_user(user_id):
    if user_id == current_user.id:
        lang = get_language()
        _t = _translations.get(lang, _translations["DE"])
        return jsonify({"error": _t["admin_err_cannot_delete_own_account"]}), 400
    db = get_db()
